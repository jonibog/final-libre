...
function loadProducts(){
  if (document.getElementById('productosTable')){
    ...
    api.open("GET", `http://localhost:8081/producto/getAll`, true);
...
function formatToTable(data){
  var thisElement = document.getElementById('productos');
  thisElement.innerHTML = "";
 // thisElement.innerHTML = thisElement.innerHTML+ "<tbody>";

  for (var x =0; x <data.length; x++){
      thisElement.innerHTML = thisElement.innerHTML + "<tr><td>" + data[x].codigo +"</td> <td>" + data[x].nombre +"</td> <td>" + data[x].stock +"</td> <td>" + "$"+data[x].precio +"</td><td><button onclick=\"funcionNoDisponible()\">Eliminar</button><button onclick=\"viewProduct('"+data[x].codigo+"')\">Ver</button></td></tr>";  
  };
  //thisElement.innerHTML = thisElement.innerHTML + "</tbody>";
}
